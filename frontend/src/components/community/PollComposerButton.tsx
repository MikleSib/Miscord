...
export function PollComposerButton({ channelId, disabled }: { channelId: number; disabled?: boolean }) {
  ...
  const openComposer = () => {
    setQuestion(''); setAnswers([{ text: '', emoji: '' }, { text: '', emoji: '' }]); setMulti(false); setDuration(86400); setError('')
    setOpen(true)
  }
  ...
  const send = async () => {
    ...
    setError('')
    try {
      addMessage(await publishPollMessage({ channelId, poll }))
      setOpen(false)
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Не удалось опубликовать опрос.')
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <>
      <button type="button" onClick={openComposer} disabled={disabled} aria-label="Создать опрос" title="Опрос" className="mr-1 flex h-11 w-11 items-center justify-center rounded-md text-text-quiet hover:bg-surface-raised hover:text-foreground disabled:opacity-40"><BarChart3 className="h-5 w-5" /></button>
      <Modal open={open} onClose={() => setOpen(false)} disableClose={sending} title="Создать опрос" contentClassName="max-w-lg bg-surface-raised">
        <div className="p-5">
          <div className="flex items-start justify-between"><div><h2 className="text-xl font-bold">Создать опрос</h2><p className="mt-1 text-sm text-text-quiet">Результаты откроются после завершения.</p></div><button type="button" onClick={() => setOpen(false)} disabled={sending} aria-label="Закрыть" className="rounded p-1 text-text-quiet hover:bg-surface disabled:opacity-50"><X className="h-5 w-5" /></button></div>
          ...
          <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setOpen(false)} disabled={sending} className="rounded-md px-4 py-2 text-sm hover:bg-surface disabled:opacity-50">Отмена</button><button type="button" onClick={() => void send()} disabled={sending || !question.trim() || validAnswers.length < 2} className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-brand-hover disabled:opacity-50">{sending ? 'Публикуем…' : 'Опубликовать'}</button></div>
        </div>
      </Modal>
    </>
  )
