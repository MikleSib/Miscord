...
import { ChevronLeft, ChevronRight, Hash, Loader2, MessageSquare as MessageSquareText, Mic2, Shield, X } from 'lucide-react'
...
export function ServerTemplateModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (serverId: number) => Promise<void> }) {
  ...
  useEffect(() => {
    if (!open) return
    setSelected(null); setPreview(null); setName(''); setError(''); setLoading(true)
