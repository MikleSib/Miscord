...
export function ServerTemplateModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (serverId: number) => Promise<void> }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Создать сервер" contentClassName="max-w-3xl bg-background">
      ...
      <div className="flex min-h-[560px] flex-col p-5">
        ...
        {loading && !templates.length ? <Loader2 className="m-auto h-7 w-7 animate-spin text-primary" /> : !selected ? (
          <div className="mt-6 min-h-0 flex-1 overflow-y-auto">
            ...
            <button type="button" onClick={() => setImporting(true)} className="mb-4 flex w-full items-center gap-4 rounded-xl border border-primary/30 bg-primary/5 px-4 py-3 text-left transition hover:border-primary/70 hover:bg-primary/10"><span className="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/15 text-primary"><Import className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-semibold">Перенести существующий сервер</span><span className="mt-0.5 block text-sm text-text-quiet">Импорт ролей, категорий, каналов, Forum и прав доступа.</span></span><ChevronRight className="h-5 w-5 text-text-quiet" /></button>
            <div className="space-y-2">{grouped.builtins.map((template, index) => { const Icon = ICONS[index % ICONS.length]; return <button key={template.id} type="button" onClick={() => void choose(template)} className="group flex w-full items-center gap-4 rounded-xl border border-border bg-surface px-4 py-3 text-left transition hover:border-primary/60 hover:bg-surface-raised"><span className="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-xl text-primary">{template.icon || <Icon className="h-5 w-5" />}</span><span className="min-w-0 flex-1"><span className="block font-semibold">{template.name}</span><span className="mt-0.5 block truncate text-sm text-text-quiet">{template.description}</span></span><ChevronRight className="h-5 w-5 text-text-quiet group-hover:text-foreground" /></button> })}</div>
