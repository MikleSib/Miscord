...
from app.core.miscord_errors import MiscordAPIError, UNKNOWN_MESSAGE
from app.db.database import get_db
from app.models import BotInteractionMessage
from app.schemas.miscord import MiscordInteractionCallback, MiscordMessageCreate, MiscordMessageUpdate
...
async def create_interaction_response(
    ...
    try:
        callback = MiscordInteractionCallback.model_validate(raw_payload)
    ...
    if message is not None:
        resource = {"type": 4, "message": miscord_message(message, guild_id=interaction.guild_id)}
    return {
        "interaction": {
...
async def get_followup_message(
    ...
    if link is None:
        raise UNKNOWN_MESSAGE()
    return miscord_message(await load_response_message(db, message_id), guild_id=interaction.guild_id)
